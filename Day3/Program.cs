...
namespace Day3
{
    class WireMap
    {
        ...
        {
            var introWireMap = new WireMap("Intro", "R8,U5,L5,D3".Split(","), "U7,R6,D4,L4".Split(","));
            introWireMap.PrintCalculations();
            var example2WireMap = new WireMap(
                "Example 2",
                "R75,D30,R83,U83,L12,D49,R71,U7,L72".Split(","),
                "U62,R66,U55,R34,D71,R55,D58,R83".Split(","));
            example2WireMap.PrintCalculations();
            var example3WireMap = new WireMap(
                "Example 3",
                "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51".Split(","),
                "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7".Split(","));
            example3WireMap.PrintCalculations();
            var problemWireMap = new WireMap(
                "Main Problem",
            ...
            problemWireMap.PrintCalculations();
        }

        private readonly Dictionary<string, int> _wireMap;
        private readonly Dictionary<string, long> _wire1Steps, _wire2Steps;
        private readonly string _name;

        public WireMap(string name, string[] wire1Route, string[] wire2Route)
        {
            _name = name;
            _wireMap = new Dictionary<string, int>();
            _wire1Steps = new Dictionary<string, long>();
            _wire2Steps = new Dictionary<string, long>();
            // set origin value explicitly to zero
            _wireMap["0,0"] = 0;
            WalkWireRoute(wire1Route, 1, _wire1Steps);
            WalkWireRoute(wire2Route, 2, _wire2Steps);
        }

        private void WalkWireRoute(IEnumerable<string> wireRoute, int wireId, Dictionary<string,long> wireSteps)
        {
//            Console.WriteLine(" Walking wire route " + wireId);
            int x = 0, y = 0;
            long steps = 0;
            foreach (var instruction in wireRoute)
            {
                var distance = long.Parse(instruction.Substring(1));
                for (long i = 0; i < distance; i++)
                {
                    ...
                    switch (instruction.Substring(0,1))
                    {
                        ...
                        case "D":
                            ...
                            break;
                        default:
                            throw new Exception("Unknown direction of travel");
                    }

                    var mapKey = x + "," + y;
                    if (_wireMap.ContainsKey(mapKey))
                    {
                        int otherWire = _wireMap[mapKey];
                        // check that wire isn't crossing itself
                        if (otherWire!=wireId)
                        {
//                            Console.WriteLine("  Found crossing at " + x + "," + y);
                            _wireMap[mapKey] += wireId;
                        }
                    }
                    else
                    {
//                        Console.WriteLine("  Setting " + x + "," + y + " to WireID: " + wireID + " and Steps: " + steps);
                        _wireMap[mapKey] = wireId;
                    }

                    // write iff it doesn't exist to make sure we keep the lowest value
                    if (!wireSteps.ContainsKey(mapKey))
                        wireSteps[mapKey] = steps;
                }
            }
        }

        public long CalculateShortestCrossingDistance()
        {
            return (from nodeIndex in this._wireMap.Keys
                where this._wireMap[nodeIndex] > 2
                // ReSharper disable once StringIndexOfIsCultureSpecific.1
                let commaPosition = nodeIndex.IndexOf(",")
                let x = long.Parse(nodeIndex.Substring(0, commaPosition))
                let y = long.Parse(nodeIndex.Substring(commaPosition + 1))
                select Math.Abs(x) + Math.Abs(y)).Concat(new[] {long.MaxValue}).Min();
        }

        public long CalculateFewestCrossingSteps()
        {
            return (from nodeIndex in this._wireMap.Keys
                where this._wireMap[nodeIndex] > 2
                select this._wire1Steps[nodeIndex] + this._wire2Steps[nodeIndex]).Concat(new[] {long.MaxValue}).Min();
        }

        public void PrintCalculations()
        {
            Console.WriteLine(_name + " wire-map shortest path: " + this.CalculateShortestCrossingDistance());
            Console.WriteLine(_name + " wire-map fewest steps: " + this.CalculateFewestCrossingSteps());
        }
    }
}